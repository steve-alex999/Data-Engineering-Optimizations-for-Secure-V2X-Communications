#!/usr/bin/env python3
"""
V2X 3-LAYER AGGREGATOR CLIENT - CSV Testing Mode (ULTRA-OPTIMIZED)
Maximum-speed version with minimal overhead
Processes packets as fast as server can handle

CSV columns: senderId, heading, speed, longAcceleration, generationTime, elevation, 
             latitude, longitude, bitLen, isAttack

isAttack values:
  0 = NORMAL (normal, no attack)
  1 = DOS_ATTACK (abnormal message intervals)
  2,3,4 = MESSAGE_FABRICATION (speed/acceleration/heading fabrication)

Classification:
  0 → NORMAL
  1 → DOS
  2,3,4 → MESSAGE_FABRICATION (consolidated)
"""
...
class AggregatorCsvClient:
    """TCP client for V2X Aggregator Server - Ultra-optimized CSV testing."""
    ...
    def send_bsm(self, bsm):
        ...
        try:
            ...
            if not response:
                # Connection lost, reconnect (NO DELAY!)
                self.socket.close()
                if not self.connect():
                    return None
                self.socket.sendall(json.dumps(bsm).encode() + b'\n')
            ...
            try:
                ...
                if not self.connect():
                    return None
                # NO ARTIFICIAL DELAY - removed time.sleep(0.05)
                self.socket.sendall(json.dumps(bsm).encode() + b'\n')
                response = self.socket.recv(4096).decode().strip()
                if not response:
                    return None
    ...
    def ground_truth_to_label(self, ground_truth: int) -> int:
        ...
        0 = NORMAL
        1 = DOS (isAttack=1)
        2 = MESSAGE_FABRICATION (isAttack=2,3,4)
        """
        if ground_truth == 0:
            return 0  # NORMAL
        elif ground_truth == 1:
            ...
            with open(self.csv_file, 'r') as f:
                ...
                for row_num, row in enumerate(csv_reader, start=2):
                    ...
                    try:
                        ...
                        # Convert ground truth to label (0=NORMAL, 1=DOS, 2=MESSAGE_FAB)
                        ...
                        if not self.fast_mode:
                            # Light accuracy check every 20 rows
                            if row_count % 20 == 0:
                                y_true = np.array(self.actual_labels)
                                y_pred = np.array(self.predictions)
                                acc = accuracy_score(y_true, y_pred)
                                
                                # Heavy metrics only every 100 rows
    ...
    def print_metrics(self, total_rows, error_count):
        ...
        class_names = ['NORMAL', 'DOS', 'MESSAGE_FABRICATION']
        print(f"\n{'Attack Type':<25} | {'Samples':<10} | {'Accuracy':<12} | {'Precision':<12} | {'Recall':<12} | {'F1-Score':<10}")
        print("-" * 100)
        ...
        for i, class_name in enumerate(class_names):
            ...
            if class_mask.sum() > 0:
                ...
                class_mask_true = (y_true == i).astype(int)
                class_mask_pred = (y_pred == i).astype(int)
                
                class_precision = precision_score(class_mask_true, class_mask_pred, zero_division=0)
                class_recall = recall_score(class_mask_true, class_mask_pred, zero_division=0)
                class_f1 = f1_score(class_mask_true, class_mask_pred, zero_division=0)
                
                print(f"{class_name:<25} | {n_samples:<10} | {class_accuracy:>10.2%} | "
                      f"{class_precision:>10.4f} | {class_recall:>10.4f} | {class_f1:>8.4f}")
        ...
        print(f"  Mean confidence:       {np.mean(y_proba):.2%}")
        print(f"  Median confidence:     {np.median(y_proba):.2%}")
        print(f"  Std deviation:         {np.std(y_proba):.4f}")
        print(f"  Min confidence:        {np.min(y_proba):.2%}")
        print(f"  Max confidence:        {np.max(y_proba):.2%}")
        
        # Confidence by correctness
        correct_mask = y_pred == y_true
        if correct_mask.sum() > 0:
            print(f"\n  Correct predictions:")
            print(f"    Mean confidence:     {np.mean(y_proba[correct_mask]):.2%}")
            print(f"    Min confidence:      {np.min(y_proba[correct_mask]):.2%}")
        
        if (~correct_mask).sum() > 0:
            print(f"\n  Incorrect predictions:")
            print(f"    Mean confidence:     {np.mean(y_proba[~correct_mask]):.2%}")
            print(f"    Max confidence:      {np.max(y_proba[~correct_mask]):.2%}")
...
def main():
    parser = argparse.ArgumentParser(
        description='V2X Aggregator Client - Ultra-Optimized CSV Testing',
        epilog="""
Examples:
  # Normal mode with real-time metrics
  python3 aggregator_client_csv_optimized.py \\
    --host 127.0.0.1 --port 5555 \\
    --csv processed_test_city2.csv

  # FAST mode: max speed, minimal output (no intermediate metrics)
  python3 aggregator_client_csv_optimized.py \\
    --host 127.0.0.1 --port 5555 \\
    --csv processed_test_city2.csv --fast
...
CSV Format (Required):
  senderId, heading, speed, longAcceleration, generationTime, elevation,
  latitude, longitude, bitLen, isAttack

Classification:
  isAttack 0 → NORMAL
  isAttack 1 → DOS
  isAttack 2,3,4 → MESSAGE_FABRICATION
        """,
    ...
    parser.add_argument('--fast', action='store_true', 
                       help='FAST mode: maximum speed, no intermediate metrics (only final results)')
    
    args = parser.parse_args()
    
    client = AggregatorCsvClient(args.host, args.port, args.csv, fast_mode=args.fast)
